    ...
    <div className="pb-10">
      ...
      <section className="mx-auto mt-14 max-w-7xl px-4 md:px-6 lg:px-8">
        <div className="glass-panel relative overflow-hidden px-5 py-8 md:px-8 md:py-10 lg:px-10">
          <div className="absolute right-4 top-1/2 hidden -translate-y-1/2 sm:block lg:right-6">
            <div className="relative h-44 w-[18rem]">
              <div className="absolute right-0 top-1/2 z-20 h-28 w-28 -translate-y-1/2 overflow-hidden rounded-full border border-boutique-gold/25 shadow-[0_18px_42px_rgba(107,15,26,0.12)] md:h-36 md:w-36 lg:h-44 lg:w-44">
                <img
                  src={contactDesk}
              ...
              <div
                className="absolute left-[3.9rem] top-1/2 z-10 h-24 w-24 -translate-y-1/2 overflow-hidden rounded-full border border-boutique-gold/20 shadow-[0_14px_34px_rgba(107,15,26,0.1)] md:left-[2.75rem] md:h-32 md:w-32 lg:left-[1.45rem] lg:h-36 lg:w-36"
                style={{
          ...
          <div className="relative mt-6 flex justify-center sm:hidden">
            <div className="relative h-28 w-[10.75rem]">
              <div className="absolute left-1/2 top-1/2 z-20 h-24 w-24 translate-x-3 -translate-y-1/2 overflow-hidden rounded-full border border-boutique-gold/25 shadow-[0_18px_42px_rgba(107,15,26,0.12)]">
                <img
                  src={contactDesk}
                  alt="Raes Boutique interior"
                  className="h-full w-full object-cover object-center"
              ...
              <div
                className="absolute left-1/2 top-1/2 z-10 h-20 w-20 -translate-x-[3.2rem] -translate-y-1/2 overflow-hidden rounded-full border border-boutique-gold/20 shadow-[0_14px_34px_rgba(107,15,26,0.1)]"
                style={{
                  WebkitMaskImage: 'radial-gradient(circle at 86% 50%, transparent 31%, black 32%)',
                  maskImage: 'radial-gradient(circle at 86% 50%, transparent 31%, black 32%)'
                }}
      ...
      <section className="mx-auto mt-20 max-w-7xl px-4 md:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-[2.25rem] bg-[#f7f0eb] px-5 py-8 shadow-[0_24px_70px_rgba(107,15,26,0.12)] md:px-8 md:py-12 lg:px-10 lg:py-14">
          ...
          <div className="relative z-10 mt-8 flex snap-x snap-mandatory items-stretch gap-2.5 overflow-x-auto pb-2 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-boutique-gold/25 md:gap-4">
            {featuredGalleryItems.map(({ id, src, product }) => (
              <Link
                key={id}
                to={`/collections/${product._id}`}
                className="group block min-w-[8.5rem] shrink-0 snap-start overflow-hidden rounded-[0.95rem] bg-white p-1.5 shadow-[0_14px_34px_rgba(107,15,26,0.07)] transition duration-500 hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(107,15,26,0.11)] md:min-w-[12.5rem] md:rounded-[1rem] md:p-2"
              >
                <div className="relative h-[7.1rem] w-full overflow-hidden rounded-[0.8rem] bg-[#f8f1eb] md:h-44 md:rounded-[0.85rem]">
                  <img
                    src={src}
                    alt=""
                    aria-hidden="true"
